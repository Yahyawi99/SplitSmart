...
export function Logo({ open }: { open: boolean }) {
  return (
    <div className="text-(--text-primary) flex justify-start items-center gap-3">
      <div
        className={`bg-(--accent-blue) flex justify-center items-center rounded-sm transition-all duration-300 ease-in-out shrink-0 ${
      ...
      >
        Split<span className="text-(--accent-btn-hover)">Smart</span>
      </h1>
    </div>
  );
}
